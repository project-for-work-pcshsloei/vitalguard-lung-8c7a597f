import { T as TSS_SERVER_FUNCTION, a as createServerFn } from "./server-C1-paTiy.js";
import { z } from "zod";
...
const analyzeRisk_createServerFn_handler = createServerRpc({
  id: "c6e7131ee654e8291980c17a496f1dba90e37f55b10d5f98911cc890905b8604",
  name: "analyzeRisk",
  filename: "src/lib/ai.functions.ts"
}, (opts) => analyzeRisk.__executeServer(opts));
const analyzeRisk = createServerFn({
  method: "POST"
}).inputValidator((input) => InputSchema.parse(input)).handler(analyzeRisk_createServerFn_handler, async ({
  data
}) => {
  const apiKey = process.env.LOVABLE_API_KEY;
  if (!apiKey) {
    return {
      analysis: "ไม่สามารถเชื่อมต่อระบบ AI ได้ในขณะนี้ กรุณาลองใหม่อีกครั้ง",
      error: "missing_key"
    };
  }
  const systemPrompt = `คุณคือ "VitalGuard Expert" ผู้เชี่ยวชาญด้านมะเร็งวิทยาที่อบอุ่น เชื่อถือได้ อธิบายเรื่องยากให้เข้าใจง่ายเป็นภาษาไทยชาวบ้าน
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (res.status === 429) return {
        analysis: "ระบบ AI กำลังใช้งานมาก กรุณาลองใหม่ในอีกสักครู่",
        error: "rate_limit"
      };
      if (res.status === 402) return {
        analysis: "เครดิต AI หมด กรุณาเติมในการตั้งค่า workspace",
        error: "credits"
      };
      return {
...
export {
  analyzeRisk_createServerFn_handler
};
